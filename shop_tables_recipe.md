...
items, item_name, unit_price, item_quantity, orders, item_id, order_date  
```
...
CREATE TABLE items_orders (
  item_id int,
  order_id int,
  constraint fk_post foreign key(item_id) references item(id) on delete cascade,
  constraint fk_tag foreign key(order_id) references order(id) on delete cascade,
...
<!-- SELECT items.id, items.name, items.unit_price, items.quantity
  FROM items 
    ...
    JOIN orderss ON items_orders.order_id = orders.id
    WHERE orders.id = 2; -->
## 5. Create the tables.

```bash
psql -h 127.0.0.1 shop_manager_test_1 < item_order.sql
```
