...
items, price, quantity, create item, list, order name, order time, order date, create order
```
...
-- The foreign key name is always {other_table_singular}_id
  item_id int,
  constraint fk_order foreign key(item_id)
    references items(id)
    on delete cascade
);
