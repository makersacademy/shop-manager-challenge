  ...
  def run
    ...
    loop do
      ...
      if user_input == "2"
        ...
        # these while loops are making sure that the user is not leaving any field blank
        item_name = ''
        while item_name.strip.empty?
          @io.puts "Please provide the item's name:"
          item_name = @io.gets.chomp
        end

        unit_price = ''
        while unit_price.strip.empty?
          @io.puts "Please provide the item's unit price:"
          unit_price = @io.gets.chomp
        end

        quantity = ''
        while quantity.strip.empty?
      ...
      if user_input == "4"
        # these while loops are making sure that the user is not leaving any field blank
        customer_name = ''
        while customer_name.strip.empty?
          @io.puts "Please provide your name:"
          customer_name = @io.gets.chomp
        end

        order_date = ''
        while order_date.strip.empty?
