...
items, name, unit price, quantity, orders, customer name, date of order 
```
...
item -> many to one -> order

Foreign key  items) - 'order_id'
...
```bash
psql -h 127.0.0.1 database_orders <orders_table.sql
```
