...
RSpec.describe Application do
    describe "run method" do
        it "prints items or orders to the terminal" do
            item = double :fake_item, id: '1', name: 'shirt'
            ...
            expect(io).to receive(:puts).with(" #1 - shirt\n")
            expect(io).to receive(:puts).with("What do you want to do?\n 1 = list all shop items\n 2 = create a new item\n 3 = list all orders\n 4 = create a new order\n q = quit").ordered
            expect(io).to receive(:gets).and_return('2').ordered
            expect(io).to receive(:puts).with("Here's a list of all current orders:").ordered
