...
describe ItemRepository do
  ...
  it "Create fails when trying to add an item with a negative unit_price or quantity" do
    item_repo = ItemRepository.new
    ...
    new_item.unit_price = -6.50
    new_item.quantity = 15
    error_message = "Cannot add an item with a negative price"
    expect { item_repo.create(new_item) }.to raise_error error_message
    ...
    new_item.unit_price = 6.50
    new_item.quantity = -15
    error_message = "Cannot add an item with a negative quantity"
    expect { item_repo.create(new_item) }.to raise_error error_message
  end
end
